...
const BADGES = [
  {
    ...
    icon: '🌱',
    name: 'First Log',
    desc: 'Log your very first carbon activity.',
  },
  {
    id: 'streak_3',
    icon: '🔥',
    name: '3-Day Streak',
    desc: 'Stay under budget for 3 days in a row.',
  },
  {
    id: 'streak_7',
    icon: '☄️',
    name: '7-Day Streak',
    desc: 'A full week under budget!',
  },
  {
    id: 'streak_30',
    icon: '🌟',
    name: '30-Day Streak',
    desc: 'A full month under budget.',
  },
  {
    id: 'budget_master',
    icon: '🎯',
    name: 'Budget Master',
    desc: 'Finish a week exactly on budget.',
  },
  {
    id: 'ton_saved',
    icon: '🐘',
    name: '1 Ton Saved',
    desc: 'Accumulate 1,000 kg of CO₂ savings.',
  },
  {
    id: 'green_warrior',
    icon: '⚔️',
    name: 'Green Warrior',
    desc: 'Complete 3 challenge packs.',
  },
  {
    id: 'forest_guard',
    icon: '🦌',
    name: 'Forest Guardian',
    desc: 'Unlock all forest animals.',
  },
  {
    id: 'lucky_draw',
    icon: '🎫',
    name: 'Lucky Winner',
    desc: 'Win a reward from the daily lottery.',
  },
];
  ...
  return `
    ...
    <div class="achievements-wrap fade-in">
      ...
      <div class="card mb-24 text-center">
        ...
        <div class="mt-16 flex justify-between items-center highlight-box">
          <div class="flex items-center gap-8">
            <span>❄️</span>
            <span style="font-weight:600; font-size:0.875rem;">Streak Freezes</span>
          </div>
          <div style="font-weight:700;">${streak.freezes}</div>
        </div>
      </div>
  ...
  import('../services/storage.js').then(async ({ unlockAchievement }) => {
    if (activities.length > 0 && !unlockedList.includes('first_log')) {
      if (await unlockAchievement('first_log')) {
        playSound('fanfare');
        fireConfetti();
        window.showToast('Achievement Unlocked: First Log! 🌱', 'success');
      }
    }
    if (streak.current >= 3 && !unlockedList.includes('streak_3')) {
      if (await unlockAchievement('streak_3')) {
        playSound('fanfare');
        fireConfetti();
        window.showToast('Achievement Unlocked: 3-Day Streak! 🔥', 'success');
      }
    }
    if (streak.current >= 7 && !unlockedList.includes('streak_7')) {
      if (await unlockAchievement('streak_7')) {
        playSound('fanfare');
        fireConfetti();
        window.showToast('Achievement Unlocked: 7-Day Streak! ☄️', 'success');
      }
    }
    if (streak.current >= 30 && !unlockedList.includes('streak_30')) {
      if (await unlockAchievement('streak_30')) {
        playSound('fanfare');
        fireConfetti();
        window.showToast('Achievement Unlocked: 30-Day Streak! 🌟', 'success');
      }
    }
  });
}
...
export function init() {
  ...
  if (btnLottery) {
    btnLottery.addEventListener('click', async () => {
      const { spendTicket, setStreak, getStreak, unlockAchievement } =
