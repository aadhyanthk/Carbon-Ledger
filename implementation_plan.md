...
- [x] **Fix `package.json` version** — change `"0.0.0"` to `"1.0.0"` *(1 min)*
...
- [x] **Delete `GRIEVANCES.md` and `SUGGESTIONS.md` from git** — `git rm` both files, add to `.gitignore` *(2 min)*
...
- [x] **Audit every `innerHTML` assignment** — grep for all `.innerHTML =` across components. Ensure every piece of dynamic user-sourced or AI-sourced data passes through `escapeHtml()` before insertion *(15 min)*
- [x] **Add Content Security Policy headers** — create/update `vercel.json` with CSP headers that restrict script sources, block inline `eval`, and limit `connect-src` to your Gemini API endpoint *(10 min)*
...
- [x] **Tree-shake Chart.js** — replace `import Chart from 'chart.js/auto'` with selective imports (`Chart, DoughnutController, ArcElement, LineController, LineElement, PointElement, LinearScale, CategoryScale, Legend, Tooltip`) and register them manually *(10 min)*
...
- [x] **Add `"test": "vitest run"` to `package.json` scripts** — without this, `npm test` fails and automated runners score 0 *(1 min, CRITICAL)*
...
- [x] **Run `npm test` and verify all tests pass** *(2 min)*

---

## 5. Accessibility (6 → 9)

- [x] **Add focus management on navigation** — after `navigate()` renders a new page in `main.js`, programmatically focus the page's `<h1>` or first heading using `viewRoot.querySelector('h1')?.focus()` with `tabindex="-1"` on headings *(10 min)*
...
- [x] **Add `aria-label` to both chart canvases** — e.g., `aria-label="Donut chart showing carbon emissions by category"` and `aria-label="Line chart showing daily carbon emissions trend"` *(2 min)*
- [x] **Add `role="img"` to the forest canvas** — with an `aria-label` like `"Living forest visualization reflecting your carbon health"` *(2 min)*
...
- [x] **Add "Built With AI" section to README** — document:
  ...
  *(15 min)*
- [x] **Add "Prompt Engineering" subsection** — brief note on how prompts evolved (e.g., "Started with simple 'parse this text' prompts, iterated to include reference emission values and strict JSON output format for reliability") *(5 min)*
...
- [x] **Add a "Did You Know?" scrolling facts section** — between the feature list and the action buttons, add an eye-catching section with shocking carbon statistics displayed as animated counter cards. Facts like:
...
- [x] **Add a "Why Track Carbon?" explainer section** — a compact, visual section explaining:
...
- [x] **Add equivalence visualizer** — a mini interactive element: *"Your daily coffee = ☕ 0.21 kg CO₂ = 🌳 4 trees needed to offset per year"*. Show 2-3 everyday items with their surprising CO₂ cost *(10 min)*

### Welcome Screen — Share CTA
- [x] **Add a "Spread the Word" section to the Welcome screen** — below the action buttons, add a visually distinct card with the text *"Help your friends track their carbon too"* with a **"Share CarbonLedger"** button *(10 min)*
- [x] **Use the Web Share API** — `navigator.share()` with a pre-written awareness message. Falls back to **copy-to-clipboard** on desktop *(10 min)*

### Settings — Persistent "Invite Friends"
- [x] **Add an "Invite Friends" row in Settings** — triggers the same Web Share / copy-link flow *(5 min)*

### Dashboard — Share Impact
- [x] **Add a "Share my impact" mini-button on the streak card** — shares a personalized message with the user's streak count *(10 min)*
...
1. [x] Add `"test": "vitest run"` to package.json
2. [x] Delete GRIEVANCES.md and SUGGESTIONS.md from git
3. [x] Add "Built With AI" section to README
4. [x] Run `npm test` and verify all pass
5. [x] **Add "Spread the Word" share feature to Welcome screen**

### 🟡 High Impact (Best ROI for time spent)
6. [x] Add "Invite Friends" to Settings + "Share impact" on Dashboard
7. [x] Add focus management on navigation
8. [x] Add `aria-label` to chart canvases and forest canvas
9. [x] Audit all `innerHTML` for `escapeHtml()` coverage
10. [x] Add CSP headers via `vercel.json`
11. [x] Tree-shake Chart.js
